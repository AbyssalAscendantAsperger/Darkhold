...
local protect_icon = {
  ...
  ["PROTECTION_PROJECTILE"] = "mods/DetailedInformation/gui/protect_icon/protection_projectile.png"
}
...
function get_protect(entity_id)
  local protect = {}
  for protext_key,_ in pairs(protect_icon) do
    protect[protext_key] = false
  end
...
function get_detailed_info(creature_id)
  ...
  local dmgModel_comp = EntityGetFirstComponent( creature_id, "DamageModelComponent" )
  if dmgModel_comp~= nil then
    for damage_type,_ in pairs(res_local_key) do
      local res = ComponentObjectGetValue2(  dmgModel_comp,"damage_multipliers", damage_type )
      if math.abs(res -1) >0.001 then
        table.insert(detailed_info.res,{type = damage_type,value = res})
      end
    end
    --print(tostring( #detailed_info.res))
  end
  --[[
  TODO
