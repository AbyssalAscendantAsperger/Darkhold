...
local function HasGreedPerk()
	return GameHasFlagRun("greed_perk")
end

local function GetGreedStacks()
...
local function GetStableTicksRequired()
	local ticks = BASE_STABLE_TICKS - GetGreedStacks()
...
local function DoRegenTick()
	...
	if missing_hp > (gap_threshold + epsilon) and gold >= turbo_cost then
