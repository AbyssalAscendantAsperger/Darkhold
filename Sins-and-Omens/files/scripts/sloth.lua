...
local function set_controls_enabled(ent, enabled)
	if not ent then return end
	local comps = EntityGetComponentIncludingDisabled(ent, "ControlsComponent")
	if comps then
		for _, comp in ipairs(comps) do
			pcall(ComponentSetValue2, comp, "enabled", enabled)
		end
	end
end
...
local function reset_ghost_state()
	if player_entity and EntityGetIsAlive(player_entity) then
		...
		set_controls_enabled(player_entity, true)
		ghost_controls_comp = nil
...
function M.OnWorldPostUpdate()
	...
	if not GameHasFlagRun("sloth_perk") then
		return
	end
	...
	if just_pressed then
		if not is_holding then
			...
			if x ~= nil then
				...
				ghost_controls_comp = EntityGetFirstComponentIncludingDisabled(player_entity, "ControlsComponent")
				set_controls_enabled(player_entity, false)
			end
		end
	end
